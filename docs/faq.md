...
attack surface is enormous. Agentic Coop DB lets you keep all of that internal
while still letting any HTTPS client send SQL.

### Why not Supabase / Hasura / PostgREST?

Those are great projects. Agentic Coop DB is intentionally smaller. It does not
...
Yes — Agentic Coop DB does not care. The endpoint is `{sql, params}`, so any layer
...
embedded in the server (`AGENTCOOPDB_MIGRATE_ON_START=true` by default) and is
also shipped as a standalone binary `agentic-coop-db-migrate`. For tenant tables,
...
be queued in the local SQLite retry queue (`agentcoopdb.queue.Queue`) and
...
runs as `agentcoopdb_owner`, which is allowed to install extensions. Make
...
Multi-region HA is not supported in v1.
