...
Agentic Coop DB is layered. Each layer depends only downward.

```
┌─────────────────────────────────────────────────────────┐
│  httpapi  (stdlib ServeMux, DTOs, error mapping)         │  transport
├─────────────────────────────────────────────────────────┤
...
The pool's login role is `agentcoopdb_gateway`. It has no privileges of its own
...
3. **Statement size** — 256 KiB cap by default.
4. **Placeholder count** — `$N` references in the AST must equal `len(params)`.
5. **Param count** — 1 000 max by default.
