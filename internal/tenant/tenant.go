...
func Setup(ctx context.Context, tx pgx.Tx, workspaceID, pgRole string, stmtTimeout, idleTimeout time.Duration) error {
	...
	// The connection-level search_path is "agentcoopdb,public" so the gateway
	// can reach control-plane tables. User SQL must target "public" only —
	// otherwise CREATE TABLE would try the agentcoopdb schema first (where
	// tenant roles have no CREATE privilege) and fail.
