...
func TestRateLimit_PerKeyBuckets(t *testing.T) {
	...
	// Make sure a whitespace-only body response is valid JSON problem body.
	req := httptest.NewRequest(http.MethodGet, "/", nil)
	w := httptest.NewRecorder()
	rl.Middleware(http.HandlerFunc(okHandler)).ServeHTTP(w, req)
	if !strings.Contains(w.Body.String(), "rate_limited") {
		t.Errorf("429 body should mention rate_limited, got: %s", w.Body.String())
	}
}
