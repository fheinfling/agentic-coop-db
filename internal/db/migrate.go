...
func RunMigrations(_ context.Context, migrationsURL, password string) (err error) {
	...
	if err := m.Up(); err != nil && !errors.Is(err, migrate.ErrNoChange) {
		// If a previous run crashed mid-migration, golang-migrate leaves the
		// version marked dirty. Auto-recover by forcing to the last clean version
		// and retrying once — the migration files are idempotent for this case.
		// This makes a single Coolify redeploy sufficient to recover without any
		// manual DB intervention or extra CLI commands.
		var dirtyErr migrate.ErrDirty
		if errors.As(err, &dirtyErr) {
			prevVersion := dirtyErr.Version - 1
			slog.Default().Warn("dirty migration state detected — auto-recovering",
				"dirty_version", dirtyErr.Version,
				"forcing_to", prevVersion,
			)
			if forceErr := m.Force(prevVersion); forceErr != nil {
				return fmt.Errorf("migrate.Up: dirty v%d; force to %d failed: %w", dirtyErr.Version, prevVersion, forceErr)
			}
			if retryErr := m.Up(); retryErr != nil && !errors.Is(retryErr, migrate.ErrNoChange) {
				return fmt.Errorf("migrate.Up: dirty v%d; retry after force also failed: %w", dirtyErr.Version, retryErr)
			}
			slog.Default().Info("migration auto-recovery succeeded", "recovered_from_version", dirtyErr.Version)
			return nil
		}
...
// ForceVersion marks the migration state as version v with dirty=false,
// without running any migration files. Use this to recover from a dirty
// database state left by a partially-applied migration — typically after
// you have fixed the migration file and want to re-run it cleanly.
//
// The canonical recovery flow for a dirty version N is:
//
//	agentic-coop-db-migrate force N-1   # reset to the last clean version
//	agentic-coop-db-migrate up           # re-run the fixed migration
func ForceVersion(_ context.Context, migrationsURL, password string, version int) (err error) {
	defer func() {
		err = redactSecrets(err, password, urlPassword(migrationsURL))
	}()
	if migrationsURL == "" {
		return errors.New("ForceVersion: empty migrations URL")
	}
	finalURL, err := injectPassword(migrationsURL, password)
	if err != nil {
		return fmt.Errorf("inject password: %w", err)
...
func EnsureOwnerRole(ctx context.Context, migrationsURL, ownerPassword string) (err error) {
	...
	if migrationsURL == "" {
		return errors.New("EnsureOwnerRole: migrations database URL is empty (set AGENTCOOPDB_MIGRATIONS_DATABASE_URL or AGENTCOOPDB_DATABASE_URL)")
	}
	...
	// Also check for the legacy aicoopdb_owner name: if it exists, migration
	// 0008 will rename it to agentcoopdb_owner. Creating a new agentcoopdb_owner
	// here first would cause that ALTER ROLE … RENAME to fail with "already exists".
	const stmt = `
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_roles
        WHERE rolname IN ('agentcoopdb_owner', 'aicoopdb_owner')
    ) THEN
...
func SetRolePassword(ctx context.Context, migrationsURL, ownerPassword, role, newPassword string) (err error) {
	...
	if strings.ContainsRune(newPassword, 0) {
		return errors.New("SetRolePassword: password contains NUL byte")
	}
	finalURL, err := injectPassword(migrationsURL, ownerPassword)
	if err != nil {
		return fmt.Errorf("inject password: %w", err)
