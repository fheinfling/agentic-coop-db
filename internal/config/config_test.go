...
func TestUsage_RendersNonEmpty(t *testing.T) {
	// Regression test for the bug where Usage() returned the result of
	// envconfig.Usage() (which is `error`, not `string`) and the file
	// failed to compile. The output should be a non-empty string and
	// should mention at least one of our env var names so we know it
	// actually rendered the spec, not an error placeholder.
	got := Usage()
	if got == "" {
